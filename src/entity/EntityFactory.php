...
final class EntityFactory{
	...
	public function register(string $className, \Closure $creationFunc, array $saveNames, ?int $legacyMcpeSaveId = null) : void{
		...
		$reflect = new \ReflectionFunction($creationFunc);
		$returnType = $reflect->getReturnType();
		if(!($returnType instanceof \ReflectionNamedType) or !is_a($returnType->getName(), $className, true)){
			throw new \InvalidArgumentException("Provided creation function must return an instance of the provided class");
		}
	...
	public function override(string $baseClass, \Closure $newCreationFunc) : void{
		...
		$reflect = new \ReflectionFunction($newCreationFunc);
		$returnType = $reflect->getReturnType();
		if(!($returnType instanceof \ReflectionNamedType) or !is_a($returnType->getName(), $baseClass, true)){
			throw new \InvalidArgumentException("Provided creation function must return an instance of the provided class");
		}
